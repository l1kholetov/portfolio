...
function calc() {
  ...
  function calcTotal() {
    ...
    if (sex === 'female') {
      ...
      result.textContent = Math.round((88.38 + 13.4 * weight + 4.8 * height - 5.7 * age) * ratio);
    }
  }
  calcTotal();
...
function cards() {
  ...
  fetch('db.json').then(function (data) {
    ...
    res.menu.forEach(function (_ref) {
      var img = _ref.img,
        altimg = _ref.altimg,
        title = _ref.title,
        descr = _ref.descr,
...
function forms(formSelector, modalTimerId) {
  ...
  function showThanksModal(message) {
    ...
    (0,_modal__WEBPACK_IMPORTED_MODULE_4__.openModal)('.modal', modalTimerId);
    ...
    setTimeout(function () {
      ...
      (0,_modal__WEBPACK_IMPORTED_MODULE_4__.closeModal)('.modal');
    }, 4000);
  }
}
...
function slider(_ref) {
  ...
  next.addEventListener('click', function () {
    if (offset == changeToNumber(width) * (slides.length - 1)) {
      offset = 0;
    } else {
      offset += changeToNumber(width);
    }
  ...
  prev.addEventListener('click', function () {
    if (offset == 0) {
      offset = changeToNumber(width) * (slides.length - 1);
    } else {
      offset -= changeToNumber(width);
    }
    slidesField.style.transform = "translateX(-".concat(offset, "px)");
  ...
  dots.forEach(function (dot) {
    dot.addEventListener('click', function (e) {
      ...
      slideIndex = slideTo;
      offset = changeToNumber(width) * (slideTo - 1);
      slidesField.style.transform = "translateX(-".concat(offset, "px)");
      checkSlidesLength();
      activeDot();
    });
  ...
  var changeToNumber = function changeToNumber(str) {
